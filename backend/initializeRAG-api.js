 ...
 * Uses API endpoints instead of direct ingestion (more efficient)
 */
async function initializeKnowledgeBase() {
  logger.info('Initializing RAG knowledge base via API...');
  ...
  for (const doc of documents) {
    try {
      ...
      if (response.ok) {
        ...
        logger.error(`Failed to ingest ${doc.docId}`, `Status: ${response.status}`);
        failCount++;
      }
    } catch (error) {
  ...
  logger.info('Knowledge base initialization complete', { successCount, failCount });
}
